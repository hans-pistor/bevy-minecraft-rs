...
fn test_render_startup_system(
    ...
    for index in 0..chunk_slice.len() {
        if  chunk_slice[index].block_id == Voxel::EMPTY_VOXEL.block_id {
            // Ignore empty voxels
            continue;
        }
        ...
        commands.spawn(PbrBundle {
            ...
            material: materials.get_handle(&block_registry.get(chunk_slice[index].block_id).material_handle),
    ...
    commands.spawn(PbrBundle {
        mesh: meshes.add(Mesh::from(shape::Cube { size: 1.0 })),
        material: materials.get_handle(&block_registry.get(Voxel::from_block_id(1).block_id).material_handle),
